...
dictionary_of_word_frequencies = {}

# For loop to iterate over each valid word
for current_valid_word in list_of_clean_valid_words:

    # Check if the word (key) already exists in the dictionary. If yes increase its count by 1
    if current_valid_word in dictionary_of_word_frequencies:

        dictionary_of_word_frequencies[current_valid_word] += 1
...
sorted_list_of_word = sorted(dictionary_of_word_frequencies.items(), key = lambda item:item[1], reverse = True)

#Since sorting a dictionary returns a list of tuple, using slicing to get the first 10 words (upto index 9)
top_10_most_frequent_words = sorted_list_of_word[0:10]

#iterating overall each word to print it in the "word -> count" form in descending order
for current_word in top_10_most_frequent_words:
